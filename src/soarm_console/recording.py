...
def build_record_config(settings: Settings, task: str, dataset_name: str) -> RecordConfig:
    if not task.strip():
...
def main() -> None:
    ...
    calibration_errors = [
        ...
        if (error := validate_calibration(path))
    ]
    if calibration_errors:
        raise SystemExit(f"Refusing record: invalid calibration: {calibration_errors}")
    missing = [
        path
        for path in (settings.leader_calibration, settings.follower_calibration)
        if not path.exists()
    ]
    if missing:
    ...
    RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
    CONTROL_PATH.unlink(missing_ok=True)
    _write_status(phase="starting", dataset_name=dataset_name, task=task)
    lerobot_record.init_keyboard_listener = _init_gui_listener
    try:
        _write_status(phase="recording")
        lerobot_record.record(build_record_config(settings, task, dataset_name))
