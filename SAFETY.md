...
읽어 에피소드의 첫 action과 견주고, 관절 하나라도 60도(집게 60%) 넘게 떨어져 있으면
**시작하지 않고** 어느 관절이 얼마나 떨어져 있는지를 적어 400으로 거절한다. 통과하면 먼저
"정렬" 단계가 지금 자세에서 첫 action까지 관절 공간에서 s-curve(smoothstep)로 보간해 가는데,
전체 이동 시간은 가장 먼 관절이 초당 20도(집게 초당 25%)를 **넘지 않도록** 정하고 최소 2초·
최대 8초로 자른다. 상한을 첨두 속도에 거는 이유는 s-curve의 최고 속도가 평균의 1.5배이기
때문이고, 60도 제한과 8초 상한이 서로 묶여 있다는 사실은 시험이 지킨다. 첫 프레임으로 뛰지
...
열지 않는다. 여기에 없는 것도 분명히 해 둔다: 충돌 회피 경로 계획이 없다. 60도 제한과 느린
정렬 속도, 그리고 **옆에 있는 사람**이 그 자리를 대신한다. 실물 확인은 사람이 옆에 있을 때
