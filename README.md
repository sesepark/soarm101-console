...
`START SOARM101` 문구로 시작함. HUBq의 기존 장치 점유 검사와 hardware doctor를 그대로
사용하므로 맥북·가상 리더·수집·정책의 제어권을 자동으로 빼앗지 않음. 두 팔은 허브 서버에
연결되어 있어야 함. 폰 화면 잠금이나 탭 이동은 물리 텔레옵을 종료하지 않으며, 탭의 정지 또는
상단 정지로 종료해야 함. 정지는 토크를 풀지 않음. 맥 앱의 native 뷰는 기존 배치를 유지함.
...
| **정책 실행** | 로컬 모델을 `lerobot-rollout`의 base + RTC로 시한 내 실행하고 콘솔 s-curve로 시작 자세에 복귀 |
...
- 콘솔 API에 인증이 없음. `127.0.0.1`에만 bind하고 밖에서 오는 것은 SSH 터널을 지나야 함. LAN에
  여는 설정은 일부러 만들지 않았음.
- **원격 추론 gRPC(Spark:8091)에는 인증이 없고 tailnet에 열려 있음.** 30Hz를 맞추려면 SSH 터널을
  뺄 수밖에 없었고(위 참조), 그 자리의 방벽은 Tailscale ACL 하나임. `0.0.0.0`이 아니라 tailnet
  주소에만 bind하므로 LAN에서는 보이지 않음. 곁다리가 살아 있는 동안(시한 600초)만 열려 있음.
