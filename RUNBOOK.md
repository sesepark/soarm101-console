...
### 1.0 온도는 토크가 걸린 팔에서만 올라온다

`Present_Temperature`를 읽기 시작하면서 잰 값(2026-09-05, 두 팔 모두 12.1V로 급전 중):

| 팔 | 토크 | 온도 |
| --- | --- | --- |
| leader | 꺼짐 | 여섯 모터 모두 `0` |
| follower | 걸림 | 37 · 40 · 37 · 40 · 40 · **50**(집게) |

**leader의 `0`은 읽기 실패가 아니다.** 두 팔 모두 ping·전압·위치가 정상으로 올라오는데
온도만 0이고, 차이는 토크뿐이다. STS3215는 구동단이 살아 있을 때만 온도를 갱신하는
것으로 보인다. 그러니 이 값은 "지금 힘을 쓰고 있는 모터가 얼마나 뜨거운가"를 묻는
용도이지, 꺼진 팔의 실온을 재는 용도가 아니다.

집게가 다른 관절보다 10도 높은 것은 자세를 유지하는 동안 계속 힘을 쓰기 때문이다.
세션 사이에 팔을 오래 세워 둘 일이 있으면 `POST /api/torque/release`로 푸는 편이 낫다
(**팔을 받친 상태에서**).
