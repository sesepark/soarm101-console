...
- **데이터 수집 파이프라인** — episode 성공/재시도 제어가 가능한 로컬 LeRobot dataset을 기록하며, Hub 자동 업로드는 하지 않습니다. 조작은 넷입니다: `right`(회차 조기 종료), `left`(다시 찍기), `esc`(저장하고 끝), **`abort`(찍던 회를 버리고 끝)**. `esc`는 루프를 빠져나온 뒤 `save_episode()`가 그대로 돌아 찍다 만 회를 저장하는데, 실제로 82프레임 2.7초짜리 조각이 온전한 시연인 척 남은 적이 있어 `abort`를 따로 두었습니다. 회차 사이 정리 구간과 저장(인코딩) 구간이 각각 `phase=resetting`·`saving`으로 나가고, 저장된 회차 수는 `episodes_saved`로 실립니다 — 정리 15초 뒤 인코딩 8초 동안 화면이 아무 말도 하지 않아 사람이 수집이 죽은 줄 알던 자리입니다. **루프가 서 있는 동안 온 키는 적용하지 않고 버립니다**(`last_control_ignored`, 아래 [회 사이에 누른 키](#회-사이에-누른-키)).
...
- **학습 서버 연동** — 수집한 데이터셋을 DGX Spark로 보내고, 학습된 체크포인트를 되받습니다. 전송은
  ...
  다만 **학습을 거는 정상 경로는 Spark 위의 sparkq 큐 하나이고**(맥 앱의 학습 버튼도 그리로 갑니다),
  이 엔드포인트는 사람이 `curl`로 여는 문으로 남아 있되 큐의 대기열을 건너뜁니다 — GPU가 하나라
  문이 둘이면 서로를 보지 못하기 때문입니다.
...
`record_loop`가 돌지 않는 동안 도착한 `right`·`left`는 **적용하지 않고 버립니다**. 그런
키를 `events`에 적으면 LeRobot의 다음 루프가 첫 반복 맨 앞에서 그것을 읽어 **다음 회차를
0프레임으로** 끝냅니다. 실제로 `test3_20260905_1413`이 그렇게 죽었습니다(2026-09-05
14:14 UTC): 회 0이 끝나고 정리 15초가 저절로 끝난 직후, 영상을 굽는 11초 동안 누른 ⏎가
회 1을 0프레임으로 만들었고, 그 빈 회를 저장하다 `validate_episode_buffer`가 `ValueError`를
내 남은 여덟 회가 통째로 사라졌습니다. 사람은 회 하나를 넘기려던 것이지 다음 회를 지우려던
것이 아닙니다. 버린 키는 `/api/status`의 `recording.runtime.last_control_ignored`에
`{key, reason: "no loop running", at}`로 실려, 화면이 "버려졌다"고 말할 수 있습니다.

`esc`와 `abort`는 이때도 받습니다 — 저장 중에 "끝내기"를 눌렀으면 그 뜻은 분명하기
때문입니다. 다만 `stop_recording`만 세우고 `exit_early`는 세우지 않습니다. 둘 다 세우면
`record()`는 다음 회를 0프레임으로 한 번 더 열고 끝납니다. `stop_recording` 하나면 저장이
끝난 뒤 `while` 조건에서 조용히 나갑니다. 루프가 시작하는 자리에서 `exit_early`를 한 번
비우는 것이 마지막 방어입니다 — 리스너를 거치지 않고 이 표를 만지는 길이 따로 있습니다
(가상 리더가 조작이 끊기면 그렇게 합니다).

빈 회차는 어떤 이유로 생기든 **저장하지 않고 건너뜁니다**. `writer.episode_buffer["size"]`가
0이면 `clear_episode_buffer()`만 부르고 `empty_episodes_skipped`를 하나 올립니다.
`episodes_saved`는 그대로입니다 — 데이터셋에 들어간 것이 없기 때문입니다. 프레임 수를
셀 수 없으면 저장합니다: 모르는 것을 0으로 읽으면 멀쩡한 회를 버립니다.

### 영상은 찍으면서 굽는다

`streaming_encoding=True`, `encoder_threads=2`로 찍습니다. 끄면 LeRobot은 프레임마다 PNG를
쓰고 `save_episode()`에서 그것을 통째로 인코딩하는데, 그동안 텔레옵 루프가 서 있고 그 정지가
바로 위 문제의 원인이었습니다. 켜면 프레임이 들어오는 즉시 별 스레드가 굽습니다.

640×480 두 대·30Hz·900프레임(30초)을 합성 프레임으로 흘려 넣고 잰 값입니다(2026-09-05,
12스레드 기계, load 1.9):
...
저장이 7.45초에서 0.41초로 줄고 넣는 속도는 사실상 그대로입니다(0.1Hz 차이는 30Hz 페이싱
루프의 잡음 안입니다). `add_frame`이 틱당 0.3ms를 더 쓰는데 33ms 예산에서 1%입니다.
합성 장면이라 실제 촬영과 정확히 같지는 않습니다 — 같은 회차를 PNG로 쌓아 구울 때 실물
로그에 남은 값은 11초였습니다. **실물에서 확인할 것**: 한 회 찍고 `loop_hz`가 29.9 그대로인지,
`camera_stale_pct`가 전과 같은지. 떨어지면 `encoder_threads=1`이 첫 번째 손잡이이고, 그래도
떨어지면 streaming을 끄고 `save_episode`의 `parallel_encoding`(카메라 두 대를 병렬로 굽는
것, 이미 기본 `True`)만 남깁니다 — 그때는 저장 대기가 돌아오므로 화면이
`saving_seconds_estimate`(직전 저장에 걸린 초)로 "약 N초"를 그립니다.
